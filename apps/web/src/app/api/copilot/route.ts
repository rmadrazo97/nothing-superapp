...
const SYSTEM_PROMPT = [
  ...
  '- When the user asks for QUANTITATIVE or COMPARABLE data (weights lifted,',
  '  macros, trends, weekly summaries, workout history, adherence, streak',
  '  progress, kcals remaining), respond by calling ONE of the render_*',
  '  tools INSTEAD of writing a markdown table, ASCII bar chart, or numeric',
  '  list. The client renders these as pixel-panel components in the',
  '  Nothing OS aesthetic — much better than text.',
  '- render_stat_ticker — one big metric with optional delta + sparkline',
  '  (current weight, kcals left, streak length, single-value readouts).',
  '- render_bar_chart — comparing categories (exercises, meals, days).',
  '  Prefer this whenever the user asks "show me a chart of X per Y" or',
  '  "compare A vs B vs C".',
  '- render_line_chart — a value over time (weight trend, adherence over',
  '  weeks). Time-series ONLY; use bar_chart for category comparisons.',
  '- render_progress_dots — X of Y (streak days elapsed, macros left,',
  '  workouts this week). Use whenever the RATIO is the story.',
  '- render_arc_graph — cyclic values (pomodoro cycle progress,',
  '  sunrise-style position). NOT for simple X-of-Y.',
  '- render_data_table — structured rows × columns. ALWAYS use this instead',
  '  of a markdown pipe table when comparing >2 attributes across items.',
  '- render_metric_grid — 3–6 related metrics in a small grid (weekly',
  '  macros summary, workout volume by category).',
