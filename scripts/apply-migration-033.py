#!/usr/bin/env python3
"""
apply-migration-033.py — one-shot script that applies 033_journal.sql to
prod using the same env / psql pattern as apply-migration-015.py.
...
def main() -> None:
    ...
    migration = REPO_ROOT / 'supabase' / 'migrations' / '033_journal.sql'
    ...
    # Confirm the table + columns + policies exist.
    ...
    if verify.returncode != 0:
        print('STDERR:', verify.stderr, file=sys.stderr)
        sys.exit(verify.returncode)
    policies = subprocess.run(
        ['psql', db_url, '-v', 'ON_ERROR_STOP=1', '-c',
         "select policyname, cmd from pg_policies "
         "where tablename='journal_entries' order by policyname;"],
        text=True, capture_output=True,
    )
    print(policies.stdout)
    if policies.returncode != 0:
        print('STDERR:', policies.stderr, file=sys.stderr)
        sys.exit(policies.returncode)
